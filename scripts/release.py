...
MODULES = [
    ...
    "tech_qt_app.py", "tech_qt_bootstrap.py", "tech_qt_backup.py", "tech_qt_cleaner.py",
    "tech_qt_drivers.py", "tech_qt_icons.py", "tech_qt_iconsync.py",
    "tech_qt_mainwindow.py", "tech_qt_mirror.py", "tech_qt_themes.py", "tech_qt_ui.py",
    "tech_qt_virustotal.py",
]
...
def _verify_source_imports():
    tool = ROOT / "tech_qt_app.py"
    text = tool.read_text(encoding="utf-8")
    imported_modules = re.findall(r"from tech_qt_\w+ import|from tech_\w+ import|import tech_qt_\w+|import tech_\w+", text)
    missing = [mod for mod in MODULES if mod.replace(".py", "") not in imported_modules]
    if missing:
        raise SystemExit(f"Missing module imports in tech_qt_app.py: {', '.join(missing)}")
    print("[verify] tech_qt_app.py imports all required modules: OK")
...
def build(obfuscated=True, qt_mode=False):
    if qt_mode:
        ...
        # Clean previous artifacts so the new module is generated from scratch.
