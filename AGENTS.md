...
6. **VERSION BUMP EVERY RELEASE.** Bump `APP_VERSION` in `tech_common.py` before every build (it drives the window title and the sidebar tool name) and make the release tag match `v<APP_VERSION>` exactly. Also update the README download link to the new version and delete older releases so only the latest remains.
