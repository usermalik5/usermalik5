...
4. **GITHUB-ONLY DATA, NO LOCAL CREDENTIALS.** User accounts (`secret.json`) and the package database (`gelotech_database_v3.json`) are NEVER stored locally or bundled into the exe. Both are fetched from the pinned update server on every login (manifest signature + DB sha256 verified); the DB is cached for the session and deleted on app close/logout/next login. `secret.json` is the LIVE accounts file: users self-register by entering their email, the app generates a PBKDF2 password, writes it back to the repo via the embedded write token, and emails it via the embedded SMTP sender. `secret.json` is therefore NOT covered by the signed manifest (database + banking list are). Passwords must stay PBKDF2 (`iters$salt$digest`) — never legacy plain SHA-256.
