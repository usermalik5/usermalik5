...
color in the list legend filters the list to that group (click again to reset).
The toolbar keeps a single **Remove Bloatware** button (its menu covers
uninstalling by UAD recommendation level — Recommended / Advanced / Expert /
Unsafe — plus Remove Popup Ads, Uninstall checked, and Clear App Data),
Restore/Backup, All, Disabled, and Filter. A permanent legend in the sidebar
explains the app removal levels (Recommended = safe to remove, Advanced =
mostly safe, Expert = may break features, Unsafe = dangerous) and the list row
colors (green = removable, orange = clean excluded, red = uninstall excluded,
...
database is pulled fresh from here on every login — users always get the latest
...
1. Fetches `secret.json` (live accounts: email + PBKDF2 hash) from GitHub —
   ...
   write token), and sends the password by email.
